...
# Definir altura e largura mínimas
    #RAFAEL - PARAMETROS
    MIN_CONTOUR_WIDTH = 10
    MIN_CONTOUR_HEIGHT = 19

# Encontrar o contorno dos veículos.
# cv2.findContours localiza objetos na imagem, mas as vezes se movimentam ou estão em locais diferentes
# cv2.RETR_EXTERNAL retorna apenas os contornos externos da imagem
# Não precisamos de todos os pontos limites armazenados,o cv2.CHAIN_APPROX_SIMPLE remove todos os pontos redundantes e comprime o contorno, economizando memória.
...
# cv2.boundingRect retângulo reto que não considera a rotação do objeto

        (x, y, w, h) = cv2.boundingRect(contour)

# Definir validade do contorno
        contour_valid = (w >= MIN_CONTOUR_WIDTH) and (h >= MIN_CONTOUR_HEIGHT)
        area = cv2.contourArea(contour)
# log armazenando dados para serem checados em caso de erro
...
# Área de objeto aumenta. Também é útil para unir partes quebradas de um objeto.
...
# Cria uma cópia do vídeo de origem
...
def main():
    ...
    print ("V: %d" % car_counter.motocycle_count)
